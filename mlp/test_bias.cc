...
int init(float** input_vector, float** bias_vector, float** output_vector_par, 
            float** output_vector_sec, size_t output_size);
...
int main(int argc, char** argv) {
    ...
    res = init(&input_vector, &bias_vector, &output_vector_par, &output_vector_sec, OUTPUTSIZE);
    
    if(res) { error_msg(res); return res; } 

    bias(input_vector, OUTPUTSIZE, bias_vector, output_vector_par);
    ...
    if(bias_vector) free(bias_vector);
...
int init(float** input_vector, float** bias_vector, float** output_vector_par, 
    ...
    *bias_vector = (float*) malloc(output_size*sizeof(float));
    res += ((*bias_vector) == NULL);  
