...
__global__ void sigmoid_par(float* input_vector, size_t N, float* output_vector);

int sigmoid(float* input_vector, size_t N, float* output_vector) {
    ...
    // kernel call
    sigmoid_par<<<num_blks, NTHREADS>>>(d_input_vector, N, d_output_vector);
...
__global__ void sigmoid_par(float* input_vector, size_t N, float* output_vector) {
    ...
    float exp_aux = exp(input_vector[th_id]);
    output_vector[th_id] = (exp_aux / (exp_aux + 1));
