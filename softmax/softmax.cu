...
int main(void) {
  ...
  int ret = compare(h_output_vector_sec, h_output_vector_par, Elems); 
  if(ret) {
...
__global__ void softmax_par1(float* input_vector, size_t N, float* maxred_vector) {
  ...
  // reduction
  for(int stride = NTHREADS; stride > 0; stride >>= 1) {
    __syncthreads();
    if(threadIdx.x < stride) {
      if(input_vector[start] < input_vector[start+stride]) input_vector[start] = input_vector[start+stride];
    }
  }
  if(threadIdx.x == 0) maxred_vector[blockIdx.x] = input_vector[start];
...
__global__ void softmax_par2(float* input_vector, size_t N, float* output_vector, float maxred, float* sum_red) {
  ...
  for(int stride = NTHREADS>>1; stride > 0; stride >>= 1) {
    __syncthreads();
    if(threadIdx.x < stride) {
      input_vector[th_id] += input_vector[th_id+stride];
    }
  }
  __syncthreads();
  if(threadIdx.x == 0) sum_red[blockIdx.x] = input_vector[th_id];
